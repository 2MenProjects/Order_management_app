...
user = Entry(frame,width=25,fg="black",bg="white",font=('Microsoft YaHei UI Light',11),border=0)
...
passwordBox = Entry(frame,width=25,fg="black",bg="white",font=('Microsoft YaHei UI Light',11),border=0)
    ...
    with open("src/database/account.json","r") as open_file:
        ...
        for line in data:
            if line["tenTaiKhoan"] == username and line["matKhau"] == password:
                win.destroy()
                mainPage.MainPage()  # Open Home Page
                flag = 1
                break
        if flag == 0:
...
signInBtn = Button(frame,width=39,pady=7,text="Sign in",bg="#57a1f8",fg="white",border=0,command=signIn,default='active')
